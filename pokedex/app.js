...
const observeLastPokemon = pokemonsOberserver => {
  const lastPokemon = document.querySelector('[data-js="pokemons-list"]').lastChild
  pokemonsOberserver.observe(lastPokemon)
}

const handleNextPokemonsRender = () => {
  const pokemonsOberserver = new IntersectionObserver(async ([lastPokemon], observer) => {
    ...
    observeLastPokemon(pokemonsOberserver)
  }, { rootMargin: '1px'})
